...
static const char *TAG1 = "Modbus_Slave1";
static const char *TAG2 = "Modbus_Slave2";
...
//		    for(;holding_reg_params.holding_data0 < MB_CHAN_DATA_MAX_VAL;) {
		        ...
		        (void)mbc_slave_check_event(mbc_slave_handle1, MB_READ_WRITE_MASK);
		        ...
		        if(reg_info.type & (MB_EVENT_HOLDING_REG_WR | MB_EVENT_HOLDING_REG_RD)) {
		            ESP_LOGI(TAG1, "HOLDING %s (%" PRIu32 " us), ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
		                            rw_str,
		                            reg_info.time_stamp,
		            ...
		            {
		                (void)mbc_slave_lock(mbc_slave_handle1);
		                holding_reg_params.holding_data0 += MB_CHAN_DATA_OFFSET;
		                if (holding_reg_params.holding_data0 >= (MB_CHAN_DATA_MAX_VAL - MB_CHAN_DATA_OFFSET)) {
		                    coil_reg_params.coils_port1 = 0xFF;
		                }
		                (void)mbc_slave_unlock(mbc_slave_handle1);
		            }
		        } else if (reg_info.type & MB_EVENT_INPUT_REG_RD) {
		            ESP_LOGI(TAG1, "INPUT READ (%" PRIu32 " us), ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
		                            reg_info.time_stamp,
		                            (unsigned)reg_info.mb_offset,
		                            (unsigned)reg_info.type,
		                            (uint32_t)reg_info.address,
		            ...
		            ESP_LOGI(TAG1, "DISCRETE READ (%" PRIu32 " us): ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
		                            reg_info.time_stamp,
		                            (unsigned)reg_info.mb_offset,
		                            (unsigned)reg_info.type,
		                            (uint32_t)reg_info.address,
		            ...
		            ESP_LOGI(TAG1, "COILS %s (%" PRIu32 " us), ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
		                            rw_str,
		                            reg_info.time_stamp,
		                            (unsigned)reg_info.mb_offset,
		                            (unsigned)reg_info.type,
...
void modbuscomm2(void *parameter)
{
	...
	//		    for(;holding_reg_params.holding_data0 < MB_CHAN_DATA_MAX_VAL;) {
			        // Check for read/write events of Modbus master for certain events
			        (void)mbc_slave_check_event(mbc_slave_handle2, MB_READ_WRITE_MASK);
			        // Get parameter information from parameter queue
			        ESP_ERROR_CHECK(mbc_slave_get_param_info(mbc_slave_handle2, &reg_info, MB_PAR_INFO_GET_TOUT));
			        const char* rw_str = (reg_info.type & MB_READ_MASK) ? "READ" : "WRITE";

			        // Filter events and process them accordingly
			        if(reg_info.type & (MB_EVENT_HOLDING_REG_WR | MB_EVENT_HOLDING_REG_RD)) {
			            ESP_LOGI(TAG2, "HOLDING %s (%" PRIu32 " us), ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
			                            rw_str,
			                            reg_info.time_stamp,
			                            (unsigned)reg_info.mb_offset,
			                            (unsigned)reg_info.type,
			            ...
			            if (reg_info.address == (uint8_t*)&holding_reg_params.holding_data0)
			            {
			                (void)mbc_slave_lock(mbc_slave_handle2);
			                holding_reg_params.holding_data0 += MB_CHAN_DATA_OFFSET;
			                if (holding_reg_params.holding_data0 >= (MB_CHAN_DATA_MAX_VAL - MB_CHAN_DATA_OFFSET)) {
			                    coil_reg_params.coils_port1 = 0xFF;
			                }
			                (void)mbc_slave_unlock(mbc_slave_handle2);
			            }
			        } else if (reg_info.type & MB_EVENT_INPUT_REG_RD) {
			            ESP_LOGI(TAG2, "INPUT READ (%" PRIu32 " us), ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
			                            reg_info.time_stamp,
			                            (unsigned)reg_info.mb_offset,
			                            (unsigned)reg_info.type,
			                            (uint32_t)reg_info.address,
			            ...
			            ESP_LOGI(TAG2, "DISCRETE READ (%" PRIu32 " us): ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
			                            reg_info.time_stamp,
			                            (unsigned)reg_info.mb_offset,
			                            (unsigned)reg_info.type,
			                            (uint32_t)reg_info.address,
			            ...
			            ESP_LOGI(TAG2, "COILS %s (%" PRIu32 " us), ADDR:%u, TYPE:%u, INST_ADDR:0x%" PRIx32 ", SIZE:%u",
			                            rw_str,
			                            reg_info.time_stamp,
			                            (unsigned)reg_info.mb_offset,
			                            (unsigned)reg_info.type,
...
//    mb_param_info_t reg_info; // keeps the Modbus registers access information
    ...
    mb_communication_info_t comm_config1 = {
            .ser_opts.port = 1,
    #if CONFIG_MB_COMM_MODE_ASCII
            .ser_opts.mode = MB_ASCII,
    #elif CONFIG_MB_COMM_MODE_RTU
    ...
    ESP_ERROR_CHECK(mbc_slave_set_descriptor(mbc_slave_handle1, reg_area));
    ESP_ERROR_CHECK(mbc_slave_set_descriptor(mbc_slave_handle2, reg_area));
    reg_area.type = MB_PARAM_HOLDING; // Set type of register area
...
//                            UART_PIN_NO_CHANGE));
    ...
    // Set UART pin numbers
      ESP_ERROR_CHECK(uart_set_pin(MB_PORT_NUM, 17,
                              18, UART_PIN_NO_CHANGE,
                              UART_PIN_NO_CHANGE));

      ESP_ERROR_CHECK(uart_set_pin(1, 4,
...
//    ESP_ERROR_CHECK(uart_set_mode(MB_PORT_NUM, UART_MODE_RS485_HALF_DUPLEX));
    ...
    xTaskCreate(modbuscomm1, "modbuscomm1", 1024*4, NULL, 8, NULL);
    xTaskCreate(modbuscomm2, "modbuscomm2", 1024*4, NULL, 8, NULL);
/*
    // The cycle below will be terminated when parameter holdingRegParams.dataChan0
    // incremented each access cycle reaches the CHAN_DATA_MAX_VAL value.
    for(;holding_reg_params.holding_data0 < MB_CHAN_DATA_MAX_VAL;) {
